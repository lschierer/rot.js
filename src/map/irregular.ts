...
export default class Irregular extends Dungeon {
	...
	_makeIrregular(room: Room): void {
		...
		const subs: { x1: number; y1: number; x2: number; y2: number }[] = [];
	...
	_carveRoom(room: Room): void {
		const subs: { x1: number; y1: number; x2: number; y2: number }[] = (room as any)._subs || [];
	...
	_connectRooms(): void {
		...
		for (let i = 0; i < this._options.extraConnections; i++) {
			...
			if (to === from) to = (to + 1) % this._rooms.length;
			// Avoid duplicates
